...
func TestList_returns_sessions_in_update_order(t *testing.T) {
	...
	if sessions[0].SessionID() != t2.SessionID() {
		t.Fatalf("expected first session to be %q (most recently updated), got %q", t2.SessionID(), sessions[0].SessionID())
	}
	if sessions[1].SessionID() != t1.SessionID() {
		t.Fatalf("expected second session to be %q, got %q", t1.SessionID(), sessions[1].SessionID())
	}
	...
	if sessions[0].SessionID() != t1.SessionID() {
		t.Fatalf("expected first session to be %q (most recently updated), got %q", t1.SessionID(), sessions[0].SessionID())
	}
}
...
func TestGet_returns_error_for_corrupted_messages_JSON(t *testing.T) {
	...
	_, err := s.db.ExecContext(ctx,
		`UPDATE sessions SET messages = '{broken json' WHERE id = ?`, sess.SessionID())
	if err != nil {
